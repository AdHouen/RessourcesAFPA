...
class Main extends Frame {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < locales.length; ++i) {
            ...
            strbuf.append("zero  group currency internat decimal monetary " +
                 "permill percent digit pattern  nan    minus infinity \n");
            strbuf.append("digit  sep   symbol  currency   sep    decimal " +
                 "                        sep           sign \n");
            ...
            strbuf.append("  "
                             ...
                             + dfs.getCurrencySymbol() + spacer
                             + dfs.getInternationalCurrencySymbol() + spacer + " "
                             + dfs.getDecimalSeparator() + spacer + "  "
                             + dfs.getMonetaryDecimalSeparator() + spacer + " "
            ...
            dfs.setCurrencySymbol("cc");
            dfs.setInternationalCurrencySymbol("ii");
            dfs.setDecimalSeparator('d');
            dfs.setMonetaryDecimalSeparator('M');
            dfs.setPerMill('p');
            dfs.setPercent('P');
            dfs.setDigit('D');
